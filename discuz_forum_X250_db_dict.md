...
| invisible | tinyint(1) |   | NO |   | 是否通过审核 0 正常 -1 回收站 -2 审核中 -3审核忽略/草稿 |
...
| status | int(10) |   | NO |   | 帖子状态 二进制位存储，对应十进制值为 #D 后的值，存储在数据库中 |
...
| displayorder | tinyint(1) |   | NO |   | 显示顺序 3，2，1 置顶 0 正常 -1 回收站 -2 审核中 -3 审核忽略 -4草稿 |
...
| status | smallint(6) unsigned |   | NO |   | 二进制位存储，对应十进制值为 #D 后的值，存储在数据库中 |
...
| 字段名 | 数据类型 | 默认值 | 允许非空 | 自动递增 | 备注 |
| ------ | -------- | ------ | -------- | -------- | ---- |
| wid | smallint(6) unsigned |   | NO | 是 | 记录id |
...
| status | tinyint(1) unsigned |   | NO |   | blog状态 1-审核 |
